...
export class ProductsService{
    ...
    async insertProduct(product:CreateProductDto, @GetUser() user):Promise<Product>{
    ...
    async getProductsWithFilter(@GetUser() user, filterDto:FindProductDto): Promise<Product[]>{
        const {search, quantity}=filterDto;
        const products = await this.getProducts(user);
        let filtered;
        if(search){
            filtered  = await products.filter(prod =>
    ...
    async updateProduct(prodId:string, changes:Product, @GetUser() user): Promise<Product>{
        const userId = user._id
        const role = user.role
        let updatedProduct;
        if(role===UserRole.USER){
    ...
    async sell(prodId:string, quant:number):Promise<Product>{
        ...
        else { throw new BadRequestException('You exceeded quantity in stock');}
    }
    ...
    async dataToCSV(data){
        const fields = ['_id','title', 'description','price','quantity','creator'];
        const csvParser = new json2csv.Parser({ fields });
        const csv = await csvParser.parse(data);
        fs.writeFile('file.csv', csv, (err) => {
            if (err) throw new InternalServerErrorException('Failed converting to csv');
        });
    }
