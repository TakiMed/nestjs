...
import { Controller, Get, Body, Post, Param, Patch, Delete, UseGuards, Query } from '@nestjs/common';
import { ProductsService } from './products.service'
import { Product } from './product.models';
import { ApiTags, ApiBody, ApiBearerAuth, ApiResponse, ApiQuery } from '@nestjs/swagger';
...
export class ProductsController {
    ...
    async addProduct(
        @Body() product:CreateProductDto, @GetUser() user): Promise<Product>{
    ...
    async getProducts(@GetUser() user , @Query() filterDto:FindProductDto):Promise<Product[]>{
    ...
    async buy(  @Param('id') prodId:string,
                @Body('quantity') newQuan:number):Promise<Product>{
    ...
    @Patch('buy/:id')
    @ApiBody({type:SellOrBuyDTO})
    async sell( @Param('id') prodId:string,
                @Body('quantity') newQuan:number):Promise<Product>{
                    const res=await this.productsService.buy(prodId,newQuan);
                    return res;
                }
    ...
    @Delete(':id')
    async removeProduct(@Param('id') prodId:string, @GetUserRole() role):Promise<string>{
